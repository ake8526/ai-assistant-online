...
import { ArrowLeft, Send, LogIn, Loader2, MapPin, FileText, Folder } from "lucide-react";
...
function ChatContent() {
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      ctx.selected = undefined; // one-shot
      applyResult(res);
    } catch (e) {
  ...
  const pickSlot = async (slot: Slot, intent?: string) => {
    ...
    if (intent === "choose_slot" && ctx.meeting) {
      // slot from find_meeting_time → book immediately with known attendees
      setBusy(true);
      ...
      // availability slot → remember; the next typed command books it
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      <header className="p-4 border-b border-slate-800 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="text-[11px] text-slate-500">{account ? account.username : "ยังไม่ได้เข้าสู่ระบบ"}</div>
        </div>
        {!account && (
          <button
            onClick={login}
            className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg bg-sky-600 hover:bg-sky-500"
          >
            <LogIn className="w-4 h-4" /> เข้าสู่ระบบ M365
          </button>
        )}
      </header>
        ...
        <div className="max-w-2xl mx-auto space-y-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={account ? "พิมพ์คำสั่ง เช่น วันนี้มีนัดอะไร…" : "เข้าสู่ระบบ M365 ก่อนเริ่มใช้งาน"}
              disabled={busy}
