...
export const SURVEY_Q: SurveyQ[] = [
  ...
  },
  {
    ...
    "what": "ถ่ายใบสั่งงานหรือไวท์บอร์ดส่งเข้า LINE ระบบอ่านตัวอักษรแล้วเสนอเป็นรายการงานให้ตรวจก่อน กดยืนยันแล้วค่อยเข้าระบบ",
    "why": "ได้อะไร: กระดาษหน้างานเข้าระบบได้เลย ไม่ต้องคีย์ใหม่ · ระบบไม่สร้างงานเองจนกว่าจะกดยืนยัน เพราะชื่อคนหรือวันที่อาจอ่านเพี้ยนได้ · ถ้ารูปอ่านไม่ออกจะบอกตรง ๆ ไม่เดาให้",
    "say": "📷 (ส่งรูปเข้ามาได้เลย)",
    "demoU": "📷 [รูปถ่าย] ใบสั่งงาน.jpg",
    "demoB": "📷 อ่านรูปแล้ว เจอ 3 งานครับ\n\n1) ตรวจปั๊ม P-12 — ผู้รับผิดชอบ: สมชาย · กำหนด: 12/09/2569\n2) เปลี่ยนไส้กรองไลน์ 2 — กำหนด: 15/09/2569\n3) แจ้งช่างซ่อมสายพาน\n\nตรวจดูก่อนนะครับ ชื่อคนหรือวันที่อาจอ่านเพี้ยนได้\nถูกแล้วกด “ยืนยันเพิ่มงาน” ด้านล่าง"
  },
  {
    "id": "b4",
    "kind": "มีอยู่แล้ว",
...
/**
 * เซสชันที่ค้างอยู่มีอายุกี่มิลลิวินาที — null คือไม่มีเซสชัน
 *
 * ตัวยิงเชิญใช้ค่านี้ตัดสินว่าจะทักซ้ำไหม: คนที่เพิ่งทำค้างไว้ต้องไม่โดนยิงใหม่
 * เพราะ startLineSurvey จะเขียนเซสชันใหม่ทับ คำตอบที่เขาตอบมาแล้วจะหายหมด
 */
export async function surveySessionAgeMs(upn: string): Promise<number | null> {
  const sess = await loadSession(upn);
  if (!sess) return null;
  const ts = typeof sess.ts === "number" ? sess.ts : 0;
  return ts ? Math.max(0, Date.now() - ts) : Number.MAX_SAFE_INTEGER;
}
...
function tryMessage(q: SurveyQ, idx: number): object {
  ...
  const tryRow = postbackRow(
    clip("➤ " + cmd, 40),
    `a=svtry&qid=${encodeURIComponent(q.id)}&i=${idx}`,
    cmd,
    "ตัวอย่างจำลอง — ไม่สั่งงานจริง"
  );
  const skipRow = postbackRow(
    "ให้คะแนนเลย",
    `a=svskip&qid=${encodeURIComponent(q.id)}&i=${idx}`,
    "ให้คะแนนเลย"
  );
...
function rateMessage(q: SurveyQ, idx: number): object {
  ...
  for (const s of SCALE) {
    const row = postbackRow(
      scaleLabel(s),
      `a=svrate&v=${s.v}&qid=${encodeURIComponent(q.id)}&i=${idx}`,
      scaleLabel(s),
      s.s
    );
...
export async function handleLineSurveyPostback(
  ...
  if (act === "svrate") {
    ...
    sess = (await loadSession(upn)) || sess;
    if (sess.phase !== "rate" || !matchesCurrentQuestion(sess, data)) return null;
    const q = SURVEY_Q[sess.idx];
    if (!q) return null;
    ...
    if (sess.idx >= SURVEY_Q.length) {
      ...
      await send(
        "reply",
        upn,
        [{ type: "text", text: scored + "\nเหลือเลือกดาวอีกนิดเดียว" }, starMessage(sess)],
        replyToken
      );
      return takeSurveyLog();
    }
