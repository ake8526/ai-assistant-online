...
export function planFor(bkk) {
  ...
  // 08:30 — เชิญทำแบบสำรวจ ฝั่งแอปตัดสินเองว่าวันนี้ใช่วันที่ตั้งไว้ไหม และส่งไปแล้วหรือยัง
  // (ตั้งวันที่ settings _survey/blast_date) ไม่ตรงวันก็จบเงียบ ๆ ไม่กินอะไร
  if (minOfDay === 8 * 60 + 30) {
    jobs.push({ label: "survey blast", path: "/api/survey/blast", timeoutMs: 110_000, background: true });
  }
