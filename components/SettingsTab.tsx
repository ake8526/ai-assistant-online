...
/** สิ่งที่ฉากโหลดตอนเปิดแอปดึงมาให้ล่วงหน้า แล้วเก็บไว้ที่ระดับแอปทั้งรอบการใช้งาน */
...
/** ค่าที่ยังไม่มา — ขีดจาง ๆ ตรงช่องนั้นช่องเดียว ไม่ใช่ซ่อนทั้งหน้าไปรอโหลด */
function Pending() {
  return <span className={`${INK_3} font-hand text-[15px]`}>—</span>;
}
...
export default function SettingsTab({
  ...
  /** ฉากโหลดตอนเปิดแอปดึงมาแล้ว ตรงนี้ยิงเฉพาะกรณีที่ตอนนั้นพลาด */
  ...
  const saveHours = async (field: "work_start" | "work_end", value: string) => {
    ...
    try {
      ...
      // เขียนไม่ผ่าน — คืนค่าเดิม ไม่ให้หน้าจอโกหกว่าบันทึกแล้ว
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-5 max-w-2xl w-full mx-auto">
      ...
      <Group title="เวลาทำงาน &amp; สถานที่" tint={N_PURPLE} tilt="rotate-[0.5deg]">
        ...
        <Row>
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-[12px] ${INK_2} truncate`}>
              {s === null ? "—" : s.work_location || "ยังไม่ตั้ง"}
            </p>
          </div>
        </Row>
        <Row last>
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-[12px] ${INK_2} truncate`}>
              {s === null ? "—" : s.home_location || "ยังไม่ตั้ง"}
            </p>
          </div>
        </Row>
      </Group>

      {/* กลุ่มนี้เป็นค่าในเครื่องล้วน ๆ ไม่เกี่ยวกับเซิร์ฟเวอร์ ใช้ได้ทันทีไม่ต้องรออะไร */}
      ...
      {(s?.error || ms?.error) && (
        <p className={`text-[12px] ${INK_2}`}>โหลดบางส่วนไม่สำเร็จ: {s?.error || ms?.error}</p>
      )}
    </div>
  );
}
